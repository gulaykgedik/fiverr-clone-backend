...
export const createGig = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const promises = files.images.map((image) => 
    uploadToCloud(next, image.path, "fiverr-gig", "image", 800, 600));
...
export const getOneGigs = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const gig = await Gig.findById(req.params.id).populate("user");

    if (!gig) return next(new NotFound("Hizmet bulunamadı"));

    res.status(200).json({ message: "Gig retrieved successfully", data: gig });
});

export const deleteGigs = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
